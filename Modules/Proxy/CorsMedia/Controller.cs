...
namespace CorsMedia
{
    public class MediaController : BaseController
    {
        ...
        WebProxy CreateProxy(string proxyValue, string proxyName)
        {
            ...
            else if (!string.IsNullOrEmpty(proxyName) && CoreInit.conf.globalproxy != null)
            {
                var settings = CoreInit.conf.globalproxy.FirstOrDefault(i => i.name == proxyName);
        ...
        {
            if (!CoreInit.conf.serverproxy.enable)
                return url;
